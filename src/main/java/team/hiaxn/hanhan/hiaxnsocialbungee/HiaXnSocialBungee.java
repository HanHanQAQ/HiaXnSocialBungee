...
import team.hiaxn.hanhan.hiaxnsocialbungee.Friends.Requests.DenyRequest;
...
public class HiaXnSocialBungee extends Plugin {
    ...
    public void onEnable() {
        ...
        saveDefaultConfig();
        RedisConnect();
        connection = MySqlManager.getNewConnection();
    ...
    public void RedisConnect() {
        jedisPubSub = new JedisPubSub() {
            @Override
            public void onMessage(String channel, String message) { //Redis监听频道信息
                /*Request指令格式
                    ...
                    if (message.split(" ")[0].equals("frequest")) {
                        switch (message.split(" ")[1]) {
                            ...
                            case "accept":
                                ...
                                break;
                            case "deny":
                                DenyRequest denyRequest = new DenyRequest(message.split(" ")[2],message.split(" ")[3]);
                        }
                    }
                }
               // ProxyServer.getInstance().broadcast("Bungee收到消息来自频道"+"---"+channel + ": " + message);
            }
            @Override
